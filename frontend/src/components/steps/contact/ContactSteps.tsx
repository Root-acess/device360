...
const BACKEND = ((import.meta.env.VITE_BACKEND_URL ?? 'http://localhost:5000') as string).replace(/\/$/, '');const IS_DEV = import.meta.env.DEV;

// ─────────────────────────────────────────────────────────────────────────────
// SINGLETON VERIFIER — lives on window, created ONCE per page load.
// Never cleared, never recreated. React cannot touch it because it is
// bound to #recaptcha-root which is outside #root in index.html.
// ─────────────────────────────────────────────────────────────────────────────
declare global {
  interface Window {
    __d360_verifier?: RecaptchaVerifier;
    __d360_verifier_ready?: boolean;
  }
}

async function ensureVerifier(): Promise<RecaptchaVerifier> {
  // Already created and rendered — reuse it
  if (window.__d360_verifier && window.__d360_verifier_ready) {
    return window.__d360_verifier;
  }

  // Create once
  if (!window.__d360_verifier) {
    window.__d360_verifier = new RecaptchaVerifier(
      auth,
      'recaptcha-root',
      {
        size: 'invisible',
        callback: () => {},
        'expired-callback': () => {
          // Mark as not ready so next call re-renders it
          window.__d360_verifier_ready = false;
        },
      }
    );
  }
  ...
  // Render once
  if (!window.__d360_verifier_ready) {
    await window.__d360_verifier.render();
    window.__d360_verifier_ready = true;
  }
...
export const PhoneStep: React.FC<PhoneStepProps> = ({ phone: init, onSubmit, goBack }) => {
  ...
  // Pre-warm the verifier while user is on phone step — so it's ready by the time OTPStep mounts
  useEffect(() => {
    if (!IS_DEV) {
      ensureVerifier().catch(() => {});
    }
  }, []);
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-5">
      ...
      <div className="flex gap-3">
        ...
        <button type="submit" disabled={phone.length !== 10 || loading}
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${phone.length === 10 && !loading ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
...
export const OTPStep: React.FC<OTPStepProps> = ({ phone, onVerify, goBack }) => {
  ...
  const hasSentRef = useRef(false); // prevent double-send in StrictMode
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      if (IS_DEV) {
        // In dev, Firebase ignores the verifier — just pass any instance
        verifier = new RecaptchaVerifier(auth, 'recaptcha-root', { size: 'invisible' });
      } else {
        verifier = await ensureVerifier();
      }
      ...
      if (err.code === 'auth/too-many-requests') {
        ...
        setError('Daily SMS quota exceeded. Try again tomorrow.');
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-5">
      <div className="text-center">
        <div className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-blue-50 flex items-center justify-center text-2xl">📱</div>
...
// ── NameStep ──────────────────────────────────────────────────────────────────
interface NameStepProps { name: string; onSubmit: (n: string) => void; goBack: () => void; }
    ...
    >
      ...
      <div className="flex gap-3">
        ...
        <button type="submit" disabled={!name.trim()}
          className={`flex-1 py-3 rounded-xl font-bold text-sm ${name.trim() ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
...
// ── AddressStep ───────────────────────────────────────────────────────────────
interface AddressFields { doorNumber: string; street: string; floor: string; landmark: string; city: string; pincode: string; }
interface AddressStepProps { address: AddressFields | string; onSubmit: (a: AddressFields) => void; goBack: () => void; }
...
export const AddressStep: React.FC<AddressStepProps> = ({ address: init, onSubmit, goBack }) => {
  ...
  const set = (k: keyof AddressFields, v: string) => setFields((f) => ({ ...f, [k]: v }));
  const canSubmit = fields.doorNumber.trim() && fields.street.trim() && fields.city.trim() && fields.pincode.length === 6;
  ...
  const autoDetect = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude, longitude } }) => {
        ...
        } catch { setLocErr('Could not fetch address. Fill manually.'); }
        finally { setLocating(false); }
      },
      () => { setLocErr('Location denied. Fill manually.'); setLocating(false); }
    );
  };
  ...
  return (
    ...
    >
      ...
      <button type="button" onClick={autoDetect} disabled={locating}
        ...
        <Navigation className="w-4 h-4" />{locating ? 'Detecting…' : 'Auto-detect my location'}
      </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input value={fields.doorNumber} onChange={(e) => set('doorNumber', e.target.value)} placeholder="e.g. 4B"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
            data-testid="address-door" required />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">Floor</label>
          <input value={fields.floor} onChange={(e) => set('floor', e.target.value)} placeholder="e.g. 3rd"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
      ...
      <div>
        ...
        <input value={fields.street} onChange={(e) => set('street', e.target.value)} placeholder="e.g. MG Road, Koramangala"
          className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
          data-testid="address-street" required />
      </div>
      <div>
        <label className="block text-xs font-semibold text-gray-600 mb-1">Landmark</label>
        <input value={fields.landmark} onChange={(e) => set('landmark', e.target.value)} placeholder="e.g. Near Cafe Coffee Day"
          className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input value={fields.city} onChange={(e) => set('city', e.target.value)} placeholder="Bengaluru"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
            data-testid="address-city" required />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">Pincode *</label>
          <input value={fields.pincode} onChange={(e) => set('pincode', e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="560001"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-blue-400 outline-none text-sm"
            data-testid="address-pincode" required />
        </div>
      </div>
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={!canSubmit}
          className={`flex-1 py-3 rounded-xl font-bold text-sm ${canSubmit ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
...
// ── TimeSlotStep ──────────────────────────────────────────────────────────────
interface TimeSlotStepProps { timeSlot: string; onSubmit: (slot: string) => void; goBack: () => void; }
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-64 overflow-y-auto">
        {slots.map((s) => (
          <button key={s.value} type="button" onClick={() => setSelected(s.value)}
            className={`py-2.5 rounded-xl border-2 font-medium text-xs transition-all ${
              selected === s.value ? 'border-blue-500 bg-blue-600 text-white' : 'border-gray-200 text-gray-700 hover:border-blue-300'
            }`}
        ...
        {slots.length === 0 && (
          <p className="col-span-full text-center text-gray-400 text-sm py-6">No slots today. Please call us.</p>
        )}
      </div>
      <div className="flex gap-3">
        <button type="button" onClick={goBack}
        ...
        <button type="submit" disabled={!selected}
          className={`flex-1 py-3 rounded-xl font-bold text-sm ${selected ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
